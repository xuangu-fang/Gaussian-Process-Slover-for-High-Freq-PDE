...
'''use init condition'''
'''GP solver class for 2d advection-like dynamics (only u_x, u_y invloved in the equation) with single kernel,
 now support advection'''


class GP_solver_2d_single_advection(object):

    # equation: g( u_{x} + u_{y} ) = f(x,y,u)
...
def get_mesh_data(u, M1, M2):
    ...
    # x_coor = np.linspace(0, 2*np.pi, num=M1)
    ...
    for equation in equation_list:
        for kernel in kernel_list:
            ...
            trick_paras = {
                ...
                'num_fold': 1,
                'tol': 1e-3,
                'other_paras': '-only-init-beta-%d' % beta,
                'beta': beta,
                'bound':'init',
            }
